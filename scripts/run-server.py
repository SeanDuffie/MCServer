...
class Server():
    ...
    def __del__(self):
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Deleting a Server object...")
        self.server_command("stop")
        time.sleep(10)
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Cancelling Process...")
        self.process.kill()
    ...
    def server_command(self, cmd: str):
        ...
        if FLAG:
            ...
            print(f"Writing Server Command: {cmd}")
